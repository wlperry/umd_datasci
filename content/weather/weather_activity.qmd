...
- **Predicting makes it stick.** Guessing the row count, the slope, or which season warms faster forces you to retrieve the idea — being surprised is when it sinks in.
- **Typing beats pasting.** Typing trains your eye for the small errors (a lowercase `temp`, `==` instead of `%in%`) that you would otherwise spend real time hunting.
:::
