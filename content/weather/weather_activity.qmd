...
```         
What dominates the plot (seasonal
cycle / long-term trend / both)?
...
Is the warming trend statistically
significant at α = 0.05?  Y / N
```
...
```         
Season   Slope (°C/yr)   Rate (°C/decade)   p-value   Signif.?
------   -------------   ----------------   -------   --------
Summer
Winter
```
...
Why might winter and summer warm at different rates? (Hint:
think about ice cover, snow reflectivity, cloud cover)
```
...
# get_GSOD(years = ..., station =
# "your-station") -> group_by(YEAR) ->
#   summarize(mean_temp = mean(TEMP, na.rm
#   = TRUE)) -> lm(mean_temp ~ YEAR)
```
