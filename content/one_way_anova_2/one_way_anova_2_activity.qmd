...
```
Match?  Y / N
Why do we prefer Anova(type = "II") as a
habit? (hint: unbalanced / two-way later)
```
...
(one-way ANOVA: F(__, __) = ____, p
____). Tukey-adjusted comparisons
...
# which pairs differ now
emmeans(sex_model, pairwise ~ species)$contrasts
```
...
n (nrow(penguins_sex)):
F(__, __) =            p =
Pairs that differ now (Adelie–Chinstrap
/ Adelie–Gentoo / Chinstrap–Gentoo):
```
