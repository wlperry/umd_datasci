...
description: >
  How to do regressions and test assumptions and interpret and calculate.
...
- **Carried forward (Lecture 04)** — shady leaves were significantly *heavier* than sunny leaves (Welch's t-test, *p* \< 0.001)
...
**We will cover:** the calibration curve with its CI band, the residuals-vs-fitted plot, and the QQ plot + Shapiro-Wilk on the residuals.

::: callout-tip
**🖐 After this chunk: Activity Parts 8–10** (plot the curve; check the residual and QQ/Shapiro assumptions).
:::
...
::: callout-note
**🔮 Predict first:** If the line fits well, what should the residuals-vs-fitted plot look like — a random flat cloud, a curve, or a funnel? Predict, then look.
:::
...
Plot the calibration curve, then check the residual plot and the QQ/Shapiro test on the residuals. Predict each diagnostic before you run it.
