---
title: "Lecture 16 — Factors: Taming Categorical Data"
...
Why alphabetical? Because a categorical column is a **factor**, and its levels default to alphabetical. Today we take control of that order — and of the labels — with the **forcats** package. You already used this kind of grouping variable back in the one-way ANOVA (Lecture 13) — today you learn to control how it displays.
:::
...
-   With factors, "predict the output" almost always means "predict the order the bars will appear in" — forcing that guess is what makes you actually think about levels instead of ggplot's defaults.
-   The `fct_` functions all take a similar shape (`fct_something(factor, ...)`), and typing them out enough times is what gets that shape into your fingers.
-   Two days of new `fct_` verbs is a lot to hold onto — using each one in the activity right after you see it in lecture is what keeps Day 1's functions available when Day 2 builds on them.
:::
::::
:::::
...
```{r default-order-10}
#| fig-format: png
#| fig-alt: "Bar chart of penguin counts by species with bars for Adelie, Chinstrap, and Gentoo appearing in alphabetical order, the default factor level order."
...
```{r infreq-10}
#| fig-format: png
#| fig-alt: "Bar chart of penguin counts by species, reordered with fct_infreq() so bars run from the most common species on the left to the least common on the right."
...
```{r reorder-10}
#| fig-format: png
#| fig-alt: "Boxplot of penguin body mass by species, with species ordered left to right from lowest to highest median body mass instead of alphabetically."
...
```{r summary-order-10}
#| fig-format: png
#| fig-alt: "Horizontal bar chart of mean body mass per penguin species, ranked from the heaviest species at the top to the lightest at the bottom."
...
::: callout-note
**🔮 Predict first:** When you fit a model — the regression from Lecture 11, or the ANOVA from Lecture 13 — R compares everything to the **first level** (Adelie here). If we make **Gentoo** the reference, what changes — the overall test, or the coefficients?
:::
...
```{r together-10}
#| fig-format: png
#| fig-alt: "Boxplot of penguin body mass by species with individual points jittered on top, species relabeled with accented names and ordered from lowest to highest median body mass."
...
Notice nothing here required touching the underlying data frame outside the pipe — `fct_recode()` and `fct_reorder()` did all the work on the fly, right where the plot is built.
::::
:::::
