...
- **Loading data** — `read_excel()`, `clean_names()` into `leaf_df`
...
```{r pipe-into-ggplot}
#| fig-format: png
#| fig-alt: "Boxplot of leaf mass in grams by shade, with jittered points overlaid, using default ggplot colors."
...
```{r scale-manual-demo}
#| fig-format: png
#| fig-alt: "Boxplot of leaf mass in grams by shade with jittered points, using custom gold and forest-green colors for sunny and shady respectively."
...
```{r facet-wrap-demo}
#| fig-format: png
#| fig-alt: "Two histograms of leaf mass in grams, one panel per shade, arranged side by side by facet_wrap."
...
```{r facet-grid-demo}
#| fig-format: png
#| fig-alt: "Two histograms of leaf mass stacked in one column, one row per shade, arranged by facet_grid."
...
```{r stat-summary-mean}
#| fig-format: png
#| fig-alt: "Jittered points of leaf mass by shade in gold and green, with a larger black point marking the mean for each group."
...
```{r stat-summary-se}
#| fig-format: png
#| fig-alt: "Jittered points of leaf mass by shade in gold and green with mean plus or minus one standard error error bars overlaid for each group."
...
- Practicing on a brand-new dataset
- `coord_cartesian()` vs. `ylim()`
- Histograms, density plots, themes, and `patchwork`
:::
:::::
