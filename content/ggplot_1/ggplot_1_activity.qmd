...
dodge_pos  <- position_dodge(width = 0.2)                # grouped means / lines: shift side by side
```
...
>
> 📌 `jitter_pos` keeps overlapping points from stacking (the `seed` makes the scatter reproducible). `dodge_pos` does nothing while there is only one group per x, but you reuse the *same* object on `geom_point()`, `stat_summary()`, and `geom_line()` so grouped summaries stay aligned.
