...
**We will cover:** counting correctly with `sum(!is.na())`, a reusable `summary_stats()` helper, and using it one group at a time.
...
# One Function, All the Basics — `summary_stats()`
...
::: {.column width="45%"}
- One call gives you `n`, `mean`, `variance`, `sd`, `se`, and a 95% CI
- `n = sum(!is.na())` — no `length()` trap
...
- **R skills:**
  ...
  - A reusable `summary_stats()` function, sourced from `themes/`
