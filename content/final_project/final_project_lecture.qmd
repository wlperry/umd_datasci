...
order: 13
week: 7
title: "Lecture 15 — Your Final Project Starts Now"
...
# Where We Left Off (Lectures 13–14)

- **`lm(Y ~ X)`** — the same linear model tool, used for both regression and ANOVA
- **One-way ANOVA** — compared body mass across three penguin species
- **`plot(model)`** — checked assumptions with base R's four diagnostic panels
- **`emmeans` post-hoc** — found *which* groups differed, not just *whether*

::: callout-note
**✅ Transition**

Every dataset we've used so far — leaf morphology, Cisco fish, Duluth weather, Palmer penguins — was handed to you.

**Today you start finding your own — data you will clean and analyze yourself.**
:::
...
- Do something no one has ever done before, with novel data or methods
...
- 🐟 **LTER Network** — Long Term Ecological Research\
  ...
  `lternet.edu/data` — see also <https://lternet.edu/using-lter-data/>
...
- Does Lake Erie lose more ice per decade than Lake Superior?
- Which lake shows the strongest trend in maximum ice cover?
- Has the timing of peak ice changed since 1973?
...
| M5 — Quarto report knitted to HTML | Every lecture — you've been doing this all semester |

You are not learning new tools for the final project — you are applying the tools you already have to data you chose.
:::
...
:::::: columns
::: {.column width="60%"}
**"I can't find a dataset I like"**

- Try Google Dataset Search first: `datasetsearch.research.google.com`
- Search for a topic you're already interested in + the word "data" or "csv"
- Come to office hours with a general topic — I can usually point you to a source in five minutes

**"My dataset is too messy to analyze"**

- Messy is fine for this course — cleaning is the first two milestones
- If you can `read_csv()` it and `glimpse()` it, you can work with it
- You don't need every column — a well-chosen `select()` is enough

**"I can't figure out what question to ask"**

- Start with the variables: what is numeric? What is categorical?
- Ask: "what would I predict, before looking at the data?"
- That prediction is your Hₐ; its opposite is H₀
:::

:::: {.column width="40%"}
**"My dataset only has two columns"**

- You need at least one X and one Y — two columns *can* be enough for regression
- But usually: look for a richer version of the same dataset on the same source

**"I already started analyzing before M1 was approved"**

- Submit M1 anyway — describe what you've done so far
- I'll either approve it or redirect you before you go further
...
**Things that do not make a strong project:**

- Many analyses, none of which are interpreted
- A significant p-value with no plot showing the actual effect
- Copied code from Stack Overflow that you can't explain
:::
...
Not complexity. Not novelty. **Clarity and reproducibility.**
