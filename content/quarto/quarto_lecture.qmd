...
- **Figures** — boxplots and mean ± SE plots, saved as PNG (GGPlot I)
...
:::: {.column width="40%"}
![Diagram of the pipe operator: leaf_df flows down into filter(shade == "sunny"), then flows down into summary(), read as "take leaf_df, then filter, then summarise."](images/diagram_pipe.png){width="90%"}
...
# 🧩 Chunk 3 of 4 · Chunk Options & Computed Values
...
# Computed values — let the chunk print them
...
- The numbers are **computed**, never typed
- Fix the data → press Render → the table fixes itself
- No copy-paste, no stale numbers in your writing
...
- **Compute values in a chunk and print them** so the report always matches the data
...
- The two-sample t-test — formally testing our leaf hypothesis
