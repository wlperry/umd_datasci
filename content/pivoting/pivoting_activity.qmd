...
ice_clean <- ice_long %>%
  mutate(
    ...
    calendar_year = if_else(str_starts(date, "Nov|Dec"),
                            year - 1, year),
...
What does the DATE of each year's peak
tell you (early vs. late season)?
```
...
In one sentence — is Lake Superior's peak ice cover
changing? How confident are you, given the scatter?
```
...
*Turn this in with your worksheet. You built the ice workflow on Lake Superior.
Now run it end-to-end on a **different Great Lake you pick** and compare trends.*
...
correct** — I want your own prediction and reasoning about *your own* lake.
:::

## E1 · Pivot a different lake (4 pts)

Pick one: `mic.txt` (Michigan), `hur.txt` (Huron), `eri.txt` (Erie), `ont.txt`
(Ontario). Change only the file name in the URL and re-run the whole workflow:

``` r
# <- your lake
url <- "https://www.glerl.noaa.gov/data/ice/glicd/daily/eri.txt"
# read + rownames_to_column -> pivot_longer (names_prefix = "X")
#   -> year to numeric, drop NA ice -> slice_max() per year
#   -> lm(ice_cover ~ year), report the slope (% per year)
```

Report your lake, its yearly-maximum ice **slope**, and — Lake Superior's is
about **−0.3 %/year** — whether your lake is losing ice **faster or slower**.

## E2 · Predict, then check — ✍️ by hand (3 pts)

**Before running E1:** your lake is shallower or deeper than Superior. Predict,
by hand, whether its max-ice trend will be steeper or shallower than Superior's,
and why. Then run E1 and write whether you were right.

## E3 · Explain it — ✍️ by hand, with YOUR slope (3 pts)

1.  Using your actual slope, explain in plain language how fast your lake is
    losing (or gaining) maximum ice cover, and over the record how much total
    change that adds up to.
2.  Shallower lakes hold less heat. Explain, in your own words, how depth could
    make one lake's ice respond faster to the same warming than another's.
