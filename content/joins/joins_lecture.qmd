...
n_distinct(captures_df$animal_id)   # includes "UNK" —
                                    # unmarked animals
```
::::
...
nrow(captures_joined)                  # every capture
                                       # event is kept
sum(is.na(captures_joined$sex))        # events we can't
                                       # attribute
```
::::
...
fisher_analysis <- captures_df %>%
  inner_join(individuals_df,
             by = c("animal_id" = "fisher_id")) %>%
  mutate(
    days_since_release = as.numeric(capture_date - release_date))

fisher_analysis %>%
  select(animal_id, sex, age_class, capture_date,
         days_since_release, mass_kg) %>%
  head()
```
::::
