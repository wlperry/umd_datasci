---
title: "Lecture — Mapping: Where Is Bigfoot?"
...
description: >
  A 2-day intro to maps in R. Spatial data and coordinate systems, the sf
  ...
  map — all with ~4,600 real Bigfoot sighting reports.
...
You can wrangle, summarize, model, and join — today those skills point at a map instead of a table. Our dataset: **~4,600 real Bigfoot sightings** across the US, each with a latitude and longitude, ready to become points and then a choropleth. 🦶
:::
...
-   A choropleth is easy to accept at face value once it's drawn — naming the state you expect to be darkest *before* rendering it is what makes you actually look at the result instead of just admiring it.
-   `st_as_sf(coords = ..., crs = ...)` and `geom_sf()` are new syntax with a specific argument order; typing them yourself is what keeps `coords = c("longitude", "latitude")` from becoming a copy-paste habit you never really learned.
-   Spatial objects, basemaps, joins, and projections each get their own chunk so a CRS mismatch doesn't get confused with a join-key mismatch later.
:::
::::
:::::
...
bigfoot_df %>%
  select(title, state, latitude, longitude, classification) %>%
  head()
```
::::
...
::: callout-note
**🔮 Predict first:** We're about to turn ~4,600 rows of lat/long into points and plot them with **no basemap**. What shape do you think the cloud of points will make?
:::
...
```{r make-sf-13}
#| fig-format: png
#| fig-alt: "Scatter plot of Bigfoot sighting points by longitude and latitude with no basemap, the point cloud tracing the outline of the continental United States."
...
```{r points-map-13}
#| fig-format: png
#| fig-alt: "Map of the lower 48 US states with Bigfoot sighting points overlaid, colored by report classification, showing denser clusters in the Pacific Northwest."
...
states_sf %>%
  ...
  left_join(count(bigfoot_df, state), by = "state")
```
...
:::: {.column width="40%"}
::: callout-important
**✅ Why show a blank choropleth?**

`left_join()` doesn't know that `"Washington"` and `"washington"` mean the same state — as far as R is concerned they're two unrelated strings, so every row fails to match and `sightings` comes back `NA` everywhere. `scale_fill_viridis_c()` then has nothing to shade with, and you get a map that ran without a single error yet shows you nothing. Grey-everywhere is the signature of a key-mismatch, not a code bug.
:::
::::
:::::
...
```{r choropleth-13}
#| fig-format: png
#| fig-alt: "Choropleth map of US states shaded by number of Bigfoot sightings using a magma color scale, with darker states indicating more reports."
# Join sightings to the state polygons ---------------
states_map <- states_sf %>%
  # drop ":north" etc.
  mutate(state = str_remove(ID, ":.*")) %>%
  left_join(sightings_by_state, by = "state")

ggplot(states_map) +
  geom_sf(aes(fill = sightings), color = "white") +
  scale_fill_viridis_c(option = "magma", direction = -1) +
  labs(title = "Bigfoot Sightings by State",
       fill = "Sightings") +
  theme_void()
```
::::
...
```{r albers-13}
#| fig-format: png
#| fig-alt: "Choropleth map of US states shaded by Bigfoot sighting counts, redrawn in an Albers equal-area projection so the country appears in its familiar proportions."
...
```{r season-13}
#| fig-format: png
#| fig-alt: "Four small maps of the lower 48 states, one per season, each showing Bigfoot sighting points in red, with summer showing the most reports."
...
- Download **your own** species from **GBIF** with the `rgbif` package and map where it lives — see the activity.
::::
:::::
