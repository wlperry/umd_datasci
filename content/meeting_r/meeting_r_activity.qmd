...
On your computer, make a new folder called `leaf_project`. Inside it, create these sub-folders:
...
In Positron: **File → Open Folder…** and pick your `leaf_project` folder.

You should see `leaf_project` appear in the Explorer panel on the left.

✏️ **Your turn:** What is the full path to your `leaf_project` folder on your computer?
...
Download both and put them in `leaf_project/data/`.
:::
...
- In Positron:
  ...
  - and save it inside `leaf_project/scripts/`.
...
shade  <- c("sunny", "shady")     # character vector
                                  # — needs quotes
...
# clean_names() tidies the headers
leaf_df <-
...
leaf_csv_df <-
  read_csv("data/2026_09_03_data_sci_leaf_area.csv") %>%
  clean_names()
```
...
> 💡 `geom_jitter()` is `geom_point()` with a little sideways nudge (`width = 0.15`) so the leaves don't stack on one line. Put it **after** `geom_boxplot()` so the points sit on top.
...
``` r
# build the plot, store it, then write
# it to figures/ at print resolution
...
ggplot(leaf_df,
       aes(x = petiole_mm, y = thickness_mm, color = shade)) +
...
```
Does petiole length predict thickness? Do
the groups cluster separately or overlap?
```
...
```
Do the slopes look similar or
different for sunny vs shady leaves?
```
...
```
What does the width of the violin
show you that a boxplot does not?
```
...
```
Do the histograms look symmetric or
skewed? Similar between sunny and shady?
```
...
scatter_plot <- ggplot(leaf_df,
                       aes(x = petiole_mm, y = thickness_mm,
                           color = shade)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "Petiole length (mm)", y = "Leaf thickness (mm)",
       color = "Side") +
  theme_bw()
...
After working through this activity your `leaf_project/` folder should contain:
...
├── scripts/
│   └── 02_leaf_analysis.R                  <- turn this in
└── figures/
