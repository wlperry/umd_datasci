...
# ── Load data ────────────────────────────────────────────────
# load csv files
tree_df <- read_excel("data/name_of_file.csv") 

# Excel files (.xlsx) — needs library(readxl)
tree_df <- read_excel("data/name_of_file.xlsx") 
...
> 💡 **Coming from Excel?** `read_excel()` is the bridge from your spreadsheet into R. Your sheet's first row becomes the **column names** in R. `clean_names()` (from `janitor`) then tidies up messy headers like `"Leaf Weight (g)"` into `leaf_weight_g` automatically.
...
tree_df <- read_excel("data/name_of_file.xlsx") %>% clean_names()
...
*Turn this in with your worksheet. In class we split leaves at an arbitrary weight. Now you split them at a number **you compute**, and reason about why that changes the picture.*

::: callout-important
**E2 and E3 must be handwritten** on paper, photographed, and embedded (`![caption](my_photo.jpg)`). Typed answers to E2/E3 get **at most half credit, even if correct** — I want your own sketch and your own reasoning about *your own* number.
:::

## E1 · Your own size cutoff (3 pts)

``` r
# 1. Compute the real mean leaf weight (don't eyeball it)
cutoff <- mean(tree_df$weight_g, na.rm = TRUE)
cutoff

# 2. Use YOUR cutoff to make a size class, then facet the boxplot
tree_df %>%
  mutate(size_class = if_else(weight_g > cutoff, "large", "small")) %>%
...
Save the figure to `figures/` at `dpi = 300`. Record your exact `cutoff`.

## E2 · Predict, then check — ✍️ by hand (2 pts)

**Before running E1:** sketch the faceted plot using your mean as the cutoff. Will the "large" and "small" panels hold roughly equal numbers of points, or not? Guess and say why. Then run it and write whether you were right, and why cutting at the mean behaves differently from cutting at a round number like 5.

## E3 · Interpret — ✍️ by hand, in your own words (2 pts)

1.  State the exact mean `weight_g` you computed.
2.  From your plots: does one side of the tree seem to have heavier leaves? Explain as if to a friend who's never seen a boxplot — describe the shapes, not the stats vocabulary.
3.  What would you want to test *formally* next, and why isn't eyeballing the boxplot enough on its own?
