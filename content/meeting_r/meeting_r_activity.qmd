...
Copy `2026_09_03_data_sci_leaf_area.csv` and `2026_09_03_data_sci_leaf_area.xlsx` into `tree_project/data/`.
...
# ── Load data ────────────────────────────────────────────────
# load csv files
tree_df <- read_csv("data/2026_09_03_data_sci_leaf_area.csv") 

# Excel files (.xlsx) — needs library(readxl)
tree_2_df <- read_excel("data/2026_09_03_data_sci_leaf_area.xlsx") 

# note the names below may vary with the names you chose
...
> 💡 **Coming from Excel?** `read_excel()` is the bridge from your spreadsheet into R. Your sheet's first row becomes the **column names** in R. `clean_names()` (from `janitor`) then tidies up messy headers like `"Leaf Mass (g)"` into `leaf_mass_g` automatically.
...
# station (USW00014913) is Minneapolis–St. Paul Airport
weather_df <- read_csv("https://www.ncei.noaa.gov/access/services/data/v1?dataset=daily-summaries&stations=USW00014913&startDate=2020-01-01&endDate=2020-12-31&format=csv")
```
...
|----------------|-----------|-------------------------------------------------------|
| `twig_id`      | character | which twig the leaf came from (e.g. `"twig_1"`)      |
| `leaf_id`      | character | leaf identifier within a twig (e.g. `"l01"`)         |
...
``` r
tree_df$mass_g    # just the mass column, as a vector
```
...
What type does R report for `shade`?   


What is the mean mass_g?   


What is the maximum thickness_mm?   
```
...
> 💡 **Two pipes — same idea:** `%>%` or `|>` — from tidyverse (we use this one throughout the course) The native pipe (a vertical bar then greater-than sign) — built into R 4.1+; you will see it in code online They behave identically for everything in this course.

``` r
# if you have odd variable names and want to fix all of them at the end you can add clean names to the end
#the %>% or |> are pipes that pass one set of information to the next command
# the pipe passes the read in data to a command to clean the variable names

tree_df <- read_excel("data/2026_09_03_data_sci_leaf_area.xlsx") %>% clean_names()
```
...
✏️ **Your turn:** Make the same plot but for `thickness_mm` instead of `mass_g`. Copy the code above and change the relevant parts.
...
``` r
# Save the mass plot — always use PNG, always use dpi = 300
mass_plot <- ggplot(tree_df, aes(x = shade, y = mass_g)) +
  geom_boxplot() +
...
``` r
ggplot(tree_df, aes(x = petiole_mm, y = thickness_mm, color = shade)) +
...
✏️ **Your turn:** Does petiole length predict thickness? Do the two groups cluster separately or overlap?
...
``` r
ggplot(tree_df, aes(x = shade, y = mass_g, fill = shade)) +
...
``` r
scatter_plot <- ggplot(tree_df, aes(x = petiole_mm, y = thickness_mm, color = shade)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE) +
