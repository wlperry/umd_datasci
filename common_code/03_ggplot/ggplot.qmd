...
A boxplot alone hides sample size. Adding jittered points shows both the distribution shape and how many observations went into each box. Define the point position **once** as a variable so every layer can reuse it:
...
> 💡 **Key idea:** Layer order matters — `geom_boxplot()` first, the points second, so the points sit **on top of** the boxes. `position_jitter(width = 0.15)` spreads overlapping points sideways; `seed = 42` freezes that random spread so the figure is identical on every render.

### Side-by-side box plots (two grouping variables)

Map a second grouping variable to `fill` to split each x-group. Now the horizontal offsets matter: dodge the boxes, and use **`position_jitterdodge()`** so the raw points land under their own box. Match `dodge.width` to the box `position_dodge()` width:
...
> 💡 **Key idea:** `position_dodge()` shifts *whole groups* apart but does not scatter points within a group; `position_jitterdodge()` does both. `position_dodge()` alone is the right choice for connected summaries (mean points, error bars, and `geom_line()` between them) — see the advanced plotting reference.
...
``` r
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  geom_point(shape = 17, color = "darkblue", size = 2.5)
```
...
| Jittered raw data | `geom_point(position = position_jitter(width = 0.15, seed = 42), alpha = 0.5)` |
| Map colour (data) | `aes(color = variable)` |
| Map shape (data) | `aes(shape = variable)` |
| Fixed colour | `geom_point(color = "darkblue")` |
...
*End of Common Code 03 — Basic ggplot2.*

*Next: Common Code 04 — Filtering and selecting data.*
