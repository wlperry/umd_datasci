...
> 💡 **Key idea:** Layer order matters. `geom_boxplot()` first and `geom_jitter()` second draws the points **on top of** the boxes. `geom_jitter()` is `geom_point()` with a small sideways nudge (`width = 0.15`) so overlapping points don't hide each other.
...
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g,
  ...
  labs(
    ...
    shape    = "Species",     # rename the shape legend
                              # (must match color label)
...
p + theme_bw()       # white background, grey
                     # gridlines, black border
p + theme_minimal()  # no background or border, subtle gridlines
p + theme_classic()  # white background, axis
                     # lines only, no gridlines
...
ggsave("figures/penguin_flipper_mass.png",
       ...
       height = 5,
       units  = "in",
       dpi    = 300)   # 300 dpi = publication
                       # quality; 96 dpi = screen only
```
...
| Raw points, 2 groups | `geom_point(position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.75))` |
