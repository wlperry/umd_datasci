---
title: "Common Code 17 — Iteration with purrr"
...
> ⬇️ **Download the companion R script:** [`17_purrr.R`](17_purrr.R)
...
``` r
numbers <- list(a = c(1,2,3,4), b = c(10,20,NA),
                c = c(5,6,7,8,9))

map(numbers, mean)                          # list of three means
map_dbl(numbers, mean)                      # named numeric
                                            # vector
...
models <- penguins |>
  drop_na() |>
  nest_by(species) |>                     # one list-row
                                          # per species
  mutate(
    fit       = list(lm(body_mass_g ~ flipper_length_mm,
                        data = data)),
...
file_paths <- list.files("data_raw/",
                         pattern = "survey_.*\\.csv",
                         full.names = TRUE)
...
map2(filenames, plots,
     ~ ggsave(.x, plot = .y, width = 5, height = 4,
              units = "in"))
```
...
``` r
# Read abundance rasters for multiple
# species → extract breeding centroid
...
*End of Common Code 17 — Iteration with purrr.*

*Next: Common Code 18 — Quarto document basics.*
