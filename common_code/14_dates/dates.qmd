...
my_data |>
  mutate(date = ymd(date_string))    # or mdy(), dmy()
                                     # depending on format
```
...
month(d)             # 6
day(d)               # 25
yday(d)              # 176  — day of year
                     # (critical for phenology)
...
```r
make_date(2026, 6, 25)                   # one date from
                                         # three numbers
make_datetime(2026, 6, 25, 14, 30, 0)   # with hour,
                                        # minute, second
...
field_visits <- tibble(
  visit = 1:4,
  date  = ymd(c("2026-04-10","2026-05-08","2026-06-03",
                "2026-07-01"))
) |>
...
ice_off <- tibble(
  year     = 1981:1995,
  date_str = c("1981-04-15", "1982-04-08", "1983-04-11",
               "1984-04-06", "1985-04-14", "1986-04-09",
               "1987-04-05", "1988-04-02", "1989-04-07",
               "1990-03-30", "1991-04-01", "1992-03-28",
               "1993-04-04", "1994-03-25", "1995-03-29")
) |>
  mutate(
    date      = ymd(date_str),
    day_of_yr = yday(date)
