...
my_function <- function(argument1, argument2) {
  ...
  return(result)   # optional — R returns the
...
# override to 1 decimal
describe_var(penguins$flipper_length_mm, 1)
```
...
std_error_safe <- function(x) {
  ...
  if (sum(!is.na(x)) < 2) {
    stop("Need at least 2 non-NA values to compute SE.",
         call. = FALSE)
  }
  sd(x, na.rm = TRUE) / sqrt(sum(!is.na(x)))
}
...
penguin_summary_plot <- function(df, group_var, response_var,
  ...
  if (!is.null(output_path)) {
    ggsave(output_path, plot = p, width = 6, height = 5,
           units = "in")
...
*Next: Common Code 17 — Iteration with purrr.*
