...
```r
select(filter(penguins, species == "Adelie"), species,
...
# Same result as the | version above,
# but scales to any number of values
...
```r
# Both of these keep rows where body
# mass is between 3500 and 4500 g
...
```r
penguins |> filter(
  (species == "Gentoo" | species == "Chinstrap") &
  body_mass_g > 4000
)
```
...
# returns FALSE, not TRUE for NA rows
penguins |> filter(sex == NA)

# ✅  Use is.na() instead
penguins |> filter(is.na(sex))       # rows where sex IS missing
penguins |> filter(!is.na(sex))      # rows where sex
                                     # is NOT missing
```
...
```r
penguins |> select(starts_with("bill"))     # bill_length_mm,
                                            # bill_depth_mm
penguins |> select(ends_with("mm"))         # all columns
                                            # ending in "mm"
```
...
```r
penguins |> select(contains("length"))      # any column
                                            # with "length"
                                            # in the name
```
...
penguins |> select(where(is.character))     # all character
                                            # columns
```
...
Use `count()` to check that your grouping variables have the right levels
and no unexpected extras:

```r
penguins |> count(species)            # rows per species
penguins |> count(species, island)    # cross-tabulate
                                      # two variables
penguins |> count(species, is.na(sex))  # how many NAs
                                        # per species?
```
...
tree_df <-
  read_excel(
    "data_raw/2026_06_25_tree_experiment_raw_data.xlsx") |>
  clean_names()
