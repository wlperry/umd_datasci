...
> You met `fct_reorder()` (order a plot by a variable) and `fct_relevel()` (set a
> model's reference group) in **One-Way ANOVA I**. This page is the rest of the
> family — renaming, merging, and lumping levels — plus `stringr` for cleaning
> the text those levels come from.
...
```r
str_trim("  sunny  ")         # "sunny"  — removes leading
                              # and trailing spaces
str_squish("sunny   shady")   # "sunny shady" — also
...
tibble(species = c("brook trout","lake trout","sculpin",
                   "walleye")) |>
  filter(str_detect(species, "trout"))

# Case-insensitive search
# TRUE
str_detect("Brook Trout", regex("trout", ignore_case = TRUE))
```
...
```r
# "bill_length" — first match
str_replace("bill_length_mm",    "_mm", "")
# "length"      — all matches
str_replace_all("length_mm_mm",  "_mm", "")
...
# (alphabetical — fine here by accident)
...
flights |>
  ...
  geom_col(fill = "darkblue", alpha = 0.7) +
  labs(
    title = "Airlines sorted by flight count (most to least)") +
  theme_minimal()
```
...
The canonical use case: species ordered from smallest to largest
on a horizontal axis. We reuse `penguins` from earlier in this file.
...
penguins |>
  ...
  labs(x = NULL, y = "Mass (g)",
       title = "Penguin species ordered by mean body mass") +
  theme_minimal() +
  theme(legend.position = "none")
```
