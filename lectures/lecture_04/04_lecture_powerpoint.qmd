...
- **Loaded** our leaf Excel file with `read_excel()`; inspected with `head()` and `glimpse()`
...
::: {.callout-note}
`car` is the Companion to Applied Regression package. It contains `leveneTest()`, which we will use to check whether our two groups have similar variance.
...
- When variances **are** equal: almost identical result to standard t-test — minimal loss of power
...
::: {.column width="60%"}
```{r formula-manual}
# Pull stats from the data and compute t by hand --------
stats_df <- tree_df %>%
  group_by(side) %>%
  summarize(
    n = sum(!is.na(weight_g)),
...
**The t-statistic tells us:** the observed difference is ~10 standard errors above zero — very far from what we'd expect if H₀ were true.

We compare this to the **t-distribution** with Welch-Satterthwaite *df* to get the *p*-value.
:::
::::
...
|---|---|
| `weight_g ~ side` | formula: compare `weight_g` between levels of `side` |
| `var.equal = FALSE` | use Welch's correction |
| `alternative = "two.sided"` | two-tailed test |

The result is a model object. We will decode each line of the output on the next slide.
:::
::::
...
The **95% CI** is the difference in means we could expect to see if we repeated this study many times.
:::
::::
...
- `subtitle` shows t and p from the model object
- R fills in the actual numbers automatically
- If you re-run with new data, the subtitle updates

::: {.callout-tip}
Embedding test statistics in the plot title or subtitle saves you from copying numbers by hand — and avoids typos in your report.
:::
:::
::::
...
- Non-overlapping bars suggest a significant difference (but overlapping bars do NOT necessarily mean no significance!)

**Rule of thumb:**

If error bars overlap by less than half the bar length, the difference is *likely* significant at α = 0.05.

The *p*-value from the t-test is the definitive answer.
:::
::::
...
ci_lo  <- round(leaf_ttest_model$conf.int[1], 2)
ci_hi  <- round(leaf_ttest_model$conf.int[2], 2)
...
- `t.test(y ~ group, data, var.equal = FALSE, alternative = "two.sided")` — the test
- `model$statistic`, `model$p.value`, `model$conf.int` — extract results
- Embedding results in `subtitle = paste0(...)` — reproducible figure labels
:::
::: {.column width="40%"}
**References:**
