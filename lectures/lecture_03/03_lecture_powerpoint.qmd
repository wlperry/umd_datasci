...
You can get data into R and make a plot.
Today we learn to *wrangle* and *describe* what we actually have.
:::
...
10 sunny + 10 shady = **balanced design**
:::
::::
...
tree_df %>% select(starts_with("weight"))
```
:::
::: {.column width="40%"}
**Why `select()` matters:**
...
To overwrite a column, just use the same name on the left:
`mutate(weight_g = round(weight_g, 1))`
:::
...
Leaves on the *shady* side of a tree will be **larger and heavier** to capture more of the limited light.
